...
BOOL CWMIReader::Initialize()
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Failed to initialize COM library. ErrorCode[0x%08X]", hr );
...
BOOL CWMIReader::SetSecurityLevels()
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Failed to initialize security. ErrorCode[0x%08X]", hr );
	}
	return TRUE;
}
...
BOOL CWMIReader::CreateLocator()
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Failed to create IWbemLocator object. ErrorCode[0x%08X]", hr );
...
BOOL CWMIReader::Connect()
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Could not connect to WMI. ErrorCode[0x%08X]", hr );
...
BOOL CWMIReader::SetProxySecurityLevels()
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Could not set proxy blanket. ErrorCode[0x%08X]", hr );
...
BOOL CWMIReader::Prepare( std::string strClass )
{
	...
	if ( FAILED( hr ) )
	{
		DEBUG( "Query for %s ErrorCode[0x%08X]", strClass.c_str(), hr );
